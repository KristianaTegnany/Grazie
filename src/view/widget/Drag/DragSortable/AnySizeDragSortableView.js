...
import { View as CView } from 'widget/Native';
...
export default class AnySizeDragSortableView extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this._indicatorDy = 0;
    this._panIndicator = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        const difference = this.state.visibleHeight > indicatorSize ? this.state.visibleHeight - indicatorSize : 0

        if(gestureState.dy > 0? difference > this._indicatorDy : this._indicatorDy > 0){
          this._panY.setValue(gestureState.dy);
          this.scrollRef && this.scrollRef.scrollTo({
            x: 0,
            y: this._panY.__getValue() * this.state.wholeHeight / this.state.visibleHeight,
            animated: false,
          });
        }
      },
  ...
  render() {
    ...
    const {selectedItem, selectedPosition} = this.state
    ...
    return (
      <View style={styles.box}>
        ...
        <ScrollView
          ...
          onLayout={({ nativeEvent: { layout: { height } } }) => this.setState({ visibleHeight: height })}
        ...
        <CView color="onSecondaryExtraDark" width={10} border={5} overflow='hidden'>
          <Animated.View {...this._panIndicator.panHandlers} onLayout={event => this._indicatorDy = event.nativeEvent.layout.y} style={{
